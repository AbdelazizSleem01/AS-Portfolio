...
export default async function sitemap() {
  ...
  try {
    ...
    const projectPages = (Array.isArray(projects) ? projects : []).map(
      (project) => ({
    ...
    const skills = await fetch(`${BASE_URL}/api/skills`).then((res) =>
      res.json()
    );
    ...
    const feedback = await fetch(`${BASE_URL}/api/feedback`).then((res) =>
      res.json()
    );
    ...
    const certificates = await fetch(`${BASE_URL}/api/Certificates`).then(
      (res) => res.json()
    );
    ...
    const subscribes = await fetch(`${BASE_URL}/api/subscribe`).then((res) =>
      res.json()
    );
